...
void main() {
    ...
    vec3 diffuse = diffIntensity * vec3(1.0, 1.0, 1.0); // Diffuse lighting component

    // Specular lighting
    vec3 viewDir = normalize(-fragPos); // Direction towards  camera
    vec3 reflectDir = reflect(-lightdir, normal);
    float specIntensity = pow(max(dot(viewDir, reflectDir), 0.0), 8.0);
    vec3 specular = specIntensity * vec3(1.0, 1.0, 1.0); // Specular lighting component
    ...
    if (isLightSource) {
        ...
        gl_FragColor = texture2D(tex, vTexCoord) * vec4(lighting, 1.0); // Set the fragment color
    }
}
